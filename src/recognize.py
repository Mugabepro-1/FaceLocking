...
from .face_lock.lock_manager import FaceLockManager
...
def main():
    ...
    lock_manager = FaceLockManager(target_identity="MuPro")  # change name to your enrolled identity
    ...
    while True:
        ...
        for f in faces:
            ...
            # 1️⃣ Try to lock the face
            lock_manager.try_lock(mr.name, mr.similarity, bbox)

            # 2️⃣ If locked, track and detect actions
            if lock_manager.is_locked():
                lock_manager.update_tracking(bbox)

            color = (0, 255, 0) if mr.accepted else (0, 0, 255)
            label = mr.name if mr.name else "Unknown"

            # show LOCK status
            if lock_manager.is_locked() and mr.name == lock_manager.target_identity:
